  ...
  setInterval(function() {
    if(temp < i) {
      ...
      yelp.business(ids[i], function(error, data) {
          if(!error) {
            json[i] = {idSent: ids[i], id: data.id, name: data.name, address: data.location.address[0], city: data.location.city, state: data.location.state_code, zip: data.location.postal_code, country: data.location.country_code, numReviews: data.review_count};
